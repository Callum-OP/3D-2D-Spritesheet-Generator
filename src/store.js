...
export const useStore = create((set) => ({
  ...
  setModelInfo: (modelInfo) =>
    set((s) => ({
      ...
      insertTime: 0,
      // Reset capture-preview UI for the new model (the rig is refit in scene.js).
      captureAngleIndex: 0,
  ...
  // ---- Capture rig (Phase 1) ----
  // The orthographic camera used to rasterize sprites. Angles orbit around the
  // model; the frustum is fit once and frozen (see three/captureCamera.js).
  captureAngleCount: 8, // number of directions (1 / 4 / 8 / custom)
  captureElevation: 20, // camera tilt above the horizon, degrees
  capturePadding: 8, // % breathing room around the model in the frame
  captureProjection: 'ortho', // 'ortho' (default) | 'perspective' (later)
  captureAngleIndex: 0, // currently-previewed direction
  capturePreview: false, // true => viewport shows the locked capture camera
  showAngleGuides: true, // draw the on-ground direction ring + labels
  modelFacing: 0, // yaw applied to the model so it faces the Front camera, degrees

  setCaptureAngleCount: (captureAngleCount) => set({ captureAngleCount }),
  setCaptureElevation: (captureElevation) => set({ captureElevation }),
  setCapturePadding: (capturePadding) => set({ capturePadding }),
  setCaptureProjection: (captureProjection) => set({ captureProjection }),
  setCaptureAngleIndex: (captureAngleIndex) => set({ captureAngleIndex }),
